...
class WebGraph():
    ...
    def make_personalization_vector(self, query=None):
        ...
        if query is None:
            ...
            #begin with a zero vector, and now substitute the entries that correspond with query with 1s
            for i in range(n):
                url = self._index_to_url(i)
                check = url_satisfies_query(url, query)
                if check == True:
                    v[0,i] = 1
    ...
    def power_method(self, v=None, x0=None, alpha=0.85, max_iterations=1000, epsilon=1e-6):
        ...
        with torch.no_grad():
            ...
            #create a vector
            #first create a zero row vector of size 1 x n
            a = torch.zeros((1,n))
            #then calculate P' rows and turn it into a dense n x 1 column vector 
            rows = torch.sparse.sum(self.P, dim=1).to_dense()
            print(rows)
            #then update the entries of a 
            a[0, rows.squeeze() != 1] = 1
            print(a)
            # main loop
            xprev = x0
            ...
            for i in range(max_iterations):
                ...
                #x = alpha * xprev * P + (alpha * xprev * a + (1-alpha)) * v

                #mat_term_1 = alpha * torch.dot(xprev1, a1)
                mat_term_1 = alpha * torch.matmul(xprev.T, a.T)

                #print(f"xprev: {xprev.shape}")
                #print(f"a: {a.shape}")
                mat_term_2 = (1 - alpha) 
                mat_scalar = mat_term_1 + mat_term_2
                #print(mat_scalar)
                add = mat_scalar * v.T
                sparse = alpha * self.P.T
                dense = xprev
                #print(mat2)
                #print(f"shape of mat2: {mat2.shape}")

                x = torch.sparse.addmm(add,sparse,dense)
                x = x / torch.norm(x)
                #x = x.T
